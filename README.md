...
|P56|删除链表中重复的结点|2019.9.26|描述：在一个排序的链表中，存在重复的结点，请删除该链表中重复的结点，重复的结点不保留，返回链表头指针<br>思路：1.处理头结点 2.处理其他结点。使用双指针做法,pPreNode 指向不重复的结点,检测pNode和pNode->next是否重复,如重复,找到 pNext,使 pPreNode指向pNext.|保留和不保留重复节点两个版本,考虑全/不/头/尾/部分重复，单/空链表.|
|P57|
